...
public class LongestWord {
    ...
    public static void main(String args[]) {
        ...
        String[] Dict = new String[] { "to", "banana", "toes", "dogs", "ababcd", "elephant" };
        ...
        char arr[] = "ogtdes".toCharArray();
        int N = arr.length;

        PrintAllWords(arr, root, N);
        int length = list.stream().max(Comparator.comparing(String::length)).get().length();

        System.out.println(list.stream().filter(s -> s.length() == length).collect(Collectors.toSet()));
